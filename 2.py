...
def print_table(nodes: np.ndarray, node_values: np.ndarray):
    ...
    table = (['x'] + x, ['y'] + y)
    # print("_" * (9 * (length + 1) - 1))
    # arrs = (nodes, node_values)
    # letters = ("x", "y")
    # for arr, letter in zip(arrs, letters):
    #     print(f"|  {letter}_i ", end="")
    #     for i in range(length):
    #         print(f"| {float(arr[i]): 4.3f} ", end="")
    #     print("|")
    #     print("_" * (9 * (length + 1) - 1))
...
def main():
    ...
    while ok in ("y", "Y"):
        ...
        if x in nodes_arr:
            ...
            # TODO: поменять slice_arr чтобы было + 1 значение
            if n != n_prev:
                if n < m:
