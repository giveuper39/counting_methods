...
def euler1_method(x_arr: list[float], y0: float, N: int, h: float) -> list[float]:
    y_arr = [y0]
    for k in range(N):
        y_arr.append(y_arr[k] + h * f(x_arr[k] + h / 2, y_arr[k] + h/2 * f(x_arr[k], y_arr[k])))
    return y_arr
...
def euler2_method(x_arr: list[float], y0: float, N: int, h: float) -> list[float]:
    y_arr = [y0]
    for k in range(N):
        y_arr.append(y_arr[k] + h / 2 * (f(x_arr[k], y_arr[k]) + f(x_arr[k] + h, y_arr[k] + h * f(x_arr[k], y_arr[k]))))
    return y_arr
...
def main():
    ...
    while ans in ("y", "Y"):
        ...
        # print("Таблица для метода Тейлора: ")
        ...
        euler2_arr = euler2_method(x_arr, y0, N, h)
