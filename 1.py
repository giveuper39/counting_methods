...
def main():
    a = -8
    b = 2
    print(f"Нахождение корней нечетной степени трансцендетного уравнения 10*cos(x) - 0.1x^2 = 0 на отрезке [{a}, {b}]")
...
def print_method_info(method: str, x: float, steps: int, root_dist: float, e: int, x0: float, x1: float = 0):
    ...
    print(
        ...
        f"Абсолютная величина невязки - {abs(f(x))}.\n")
...
def root_separation(x_arr: np.ndarray, y_arr: np.ndarray, num_segments: int) -> tuple[tuple[Any, Any], ...]:
    ...
    for i in range(num_segments - 1):
        ...
        # print(y1, y2, m := np.multiply(y1, y2), flag := m < 0)
        if y1 * y2 < 0:
            # print(x1, x2, y1, y2)
